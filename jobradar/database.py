...
from sqlalchemy import String, Text, DateTime, Integer, Boolean, UniqueConstraint, select
...
class Job(Base):
    ...
    id: Mapped[int]=mapped_column(primary_key=True); canonical_job_key: Mapped[str]=mapped_column(String(128), index=True); provider: Mapped[str]=mapped_column(String(32)); external_job_id: Mapped[str]=mapped_column(String(255))
    ...
    live_status: Mapped[str]=mapped_column(String(16), default="UNCERTAIN"); notified: Mapped[bool]=mapped_column(Boolean, default=False, index=True); first_seen_at: Mapped[datetime]=mapped_column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
class Resume(Base):
